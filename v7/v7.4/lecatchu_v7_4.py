...
class LeCatchu_Engine: # LeCatchu LehnCATH4 Engine
    def __init__(self, sboxseed="Lehncrypt", encoding_type="packet", data=""):
        if len(data) != 0:
    ...
    def decrypt(self, bytestarget, key, xbase=1):
        keygen = self.hash_stream(key, xbase)
        return bytes([(bytestarget[i]-next(keygen))%256 for i in range(len(bytestarget))])
    def encrypt_with_iv(self, bytestarget, key, xbase=1, ivlength=256):
    	return self.encrypt(self.addiv(bytestarget, ivlength), key, xbase=1)
    def decrypt_with_iv(self, bytestarget, key, xbase=1, ivlength=256):
    	return self.deliv(self.decrypt(bytestarget, key, xbase=1), ivlength)
    ...
    def addiv(self, data, length=256): # IV/nonce (Initialization Vector) Add IV
    	key = os.urandom(length)
    	return key+self.encrypt(data, key, xbase=1)
    def deliv(self, data, length=256): # Remove IV
    	key = data[:length];data = data[length:]
    	return self.decrypt(data, key, xbase=1)
